...
收到 `{"type":"ota_available"}` 时只检查版本信息，不会自动下载或重启。

发现新版本后，待机页右上角会显示下载图标。板载 KEY（GPIO18）操作顺序：

1. 第一次短按：显示当前版本和可用版本。
2. 第二次短按：确认下载；下载完成后校验 SHA-256 与 RSA-3072 签名。
3. 下载完成后再次短按：确认安装并重启。

版本详情 20 秒无操作会返回待机页，更新图标仍保留。已下载的固件和
“等待安装”状态会写入 NVS，即使断电重启也不会自行安装或丢失确认状态。
局域网页面 `http://viora.local/ota` 提供相同的“检查 → 下载 → 安装”三段操作。
