...
完整生成提示词与输出来源记录在 `expression-generation.json`；第 1 张后续眼睛修正记录在 `idle-eye-refinement.json`。第 11 张说话嘴型已改成留白的弧形笑嘴，去掉黑色嘴腔和舌头，修正记录见 `speaking-mouth-refinement.json`。
...
- 思考：使用不同长度的停顿循环。
- 说话：第 12 / 11 / 12 / 11 / 12 张按 560 / 220 / 420 / 280 / 760 ms 循环；短暂张开弧形笑嘴，其余时间保持浅笑。未做音素口型同步。
