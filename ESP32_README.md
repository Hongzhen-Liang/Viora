...
1. **传感器（I2C）**：板载 `SHTC3` 应显示 `OK`；外接 `BH1750` 未连接时
   显示 `FAIL` 属正常。扩展 I2C 使用 SDA=GPIO13、SCL=GPIO14。随后每 30 秒打印一行
   `[SENSOR] temp=..C hum=..% (sht_raw=..C/..% offset=..C) ...`。
2. **LD2410S**：有人进入应打印 `[PRESENCE] 有人进入，距离=...cm`，离开并
   持续 8 秒后打印 `[PRESENCE] 已确认无人`。驱动同时兼容出厂默认极简帧和
   标准数据帧，并打印当前格式；UART 超过 3 秒无有效帧时自动回退到 OT2。
   待机页右上角仅在有人时显示小型线稿人物图标，不显示文字或距离。
...
- 人在状态需连续有人约 0.8 秒才进入，连续无人 8 秒才离开；只有设备实际观察过
  一次离开、且离开至少 30 分钟，回来后在 1.8 米范围连续停留 3 秒才允许主动问候。
  主动问候每 12 小时最多一次，冷却时间写入 NVS，重启不会清零；22:00～08:00
  或系统尚未校时均保持安静；返回 2 分钟内未满足条件则放弃本次问候，主动呼叫
  期间确认用户离开也会立即停止，且仅在设备待机、联网、未 OTA 时触发。
