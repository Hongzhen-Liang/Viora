...
void audio_play_drain();                               // 播放任务的一次喂数；仅任务创建失败时由主循环兜底
bool audio_play_task_running();                        // 独立 I2S 播放任务是否已启动
...
void audio_set_volume(float vol);                      // 播放音量 0.1~1.0（LLM volume_up/down 调用）
